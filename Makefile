...
$(CHROME_ZIP):
	mkdir -p $(CHROME_DIR)
	zip -r $(CHROME_ZIP) . -x "dist/*" ".git/*" "scripts/*" "background_safari.js"

firefox: $(FIREFOX_XPI)

$(FIREFOX_XPI):
	web-ext build --artifacts-dir $(FIREFOX_DIR)  --ignore-files dist/** --ignore-files scripts/** --ignore-files .git/** --ignore-files background_safari.js
